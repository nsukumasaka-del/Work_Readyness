...
import { createEmailChallenge, verifyEmailChallenge } from "../lib/auth-otp";
import { createAdminNotification, writeAuditLog } from "../lib/admin-ops";
import {
  PLAN_CATALOG,
  PROGRAMME,
...
router.post("/career/signup", async (req, res) => {
  ...
  const targetRole = req.body?.targetRole ? String(req.body.targetRole).trim() : undefined;
...
router.post("/career/login", async (req, res) => {
  ...
  if (admin) {
    ...
    await db
      .update(profilesTable)
      .set({ lastLoginAt: new Date(), status: profile.status || "active" })
    ...
    const [{ value: profileCount }] = await db.select({ value: count() }).from(profilesTable);
    req.log.info({ profileId: profile.id, adminId: admin.id }, "Admin logged in");
    res.json({
  ...
  const [{ value: profileCount }] = await db.select({ value: count() }).from(profilesTable);
  req.log.info({ profileId: profile.id }, "User logged in");
  res.json({
    ...toProfileResponse(profile, profileCount),
    isAdmin: false,
  });
...
router.post("/career/auth/verify", async (req, res) => {
  ...
  let verified: { email: string; purpose: "signup"; payload: Record<string, unknown> };
  try {
    verified = await verifyEmailChallenge({ challengeId, code });
  } catch (error) {
  ...
  const phone = verified.payload.phone ? String(verified.payload.phone) : undefined;
  const location = verified.payload.location ? String(verified.payload.location) : undefined;
  const targetRole = verified.payload.targetRole ? String(verified.payload.targetRole) : undefined;
  ...
  if (profile) {
    const [updated] = await db
      .update(profilesTable)
      .set({ name, phone, location, targetRole, passwordHash })
      .where(eq(profilesTable.id, profile.id))
      .returning();
    profile = updated;
  } else {
    const [created] = await db
      .insert(profilesTable)
      .values({ name, email, phone, location, targetRole, passwordHash })
      .returning();
    profile = created;
  }

  const [{ value: profileCount }] = await db.select({ value: count() }).from(profilesTable);
  req.log.info({ profileId: profile.id }, "User signed up after OTP");
  try {
